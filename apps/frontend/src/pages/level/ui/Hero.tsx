...
export default function Hero({ imageSrc, header, title, children, className }: Props) {
    return (
        <section className={clsx('hero', className)}>
            <img className="hero__img" src={imageSrc} alt="" />
            <div className="hero__shade" />
            <div className="hero__content">
                {header ? <HeroHeading {...header} /> : title ? <h1 className="hero__legacyTitle">{title}</h1> : null}
                {children}
            </div>
        </section>
    )
