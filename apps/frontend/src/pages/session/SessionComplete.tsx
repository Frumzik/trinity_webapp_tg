...
type Track = { id: number | string; title: string; subtitle?: string; artworkUrl?: string };
...
export default function SessionComplete() {
  ...
  const t = state?.track;
  const minutes = Math.max(1, Math.round(((state?.duration ?? 0) as number) / 60));

  const hasQueue = Array.isArray(state?.queue) && (state!.queue as any[]).length > 0;
  const idx = typeof state?.index === "number" ? state!.index! : undefined;
  const nx = typeof state?.nextIndex === "number" ? state!.nextIndex! : (typeof idx === "number" && hasQueue ? idx + 1 : undefined);
  const hasNext = hasQueue && typeof nx === "number" && nx! >= 0 && nx! < (state!.queue as any[]).length;
  ...
  return (
    <div className="session session--complete">
      ...
      <main className="session__main">
        {t?.artworkUrl && <img className="session__thumb" src={t.artworkUrl} alt="" />}
