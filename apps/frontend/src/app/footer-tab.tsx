...
const FooterTabCtx = createContext<{tab: Tab; setTab: (t: Tab)=>void} | null>(null)
...
export function FooterTabProvider({ children }: { children: ReactNode }) {
    ...
    return <FooterTabCtx.Provider value={useMemo(() => ({ tab, setTab }), [tab])}>{children}</FooterTabCtx.Provider>
}

export function useFooterTab() {
    const ctx = useContext(FooterTabCtx)
    if (!ctx) throw new Error('useFooterTab must be used within FooterTabProvider')
    return ctx
}
