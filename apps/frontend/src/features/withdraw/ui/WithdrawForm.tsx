...
export default function WithdrawForm({ quote, submit }: Props) {
    ...
    return (
        <div className="wform">
            ...
            <input className="wform__input" value={amount} onChange={e => setAmount(Number(e.target.value.replace(/\D/g, '') || 0))} />
            <QuickAmounts options={recom} onPick={setAmount} />
            {q && <div className="wform__fee">Комиссия составит {q.fee.fixed} $ (фиксированная {q.fee.percent}%)</div>}
            <input className="wform__addr" placeholder="Вставьте адрес USDT BEP 20" value={addr} onChange={e => setAddr(e.target.value)} />
            <div className="wform__agree">Нажимая кнопку «вывести», я подтверждаю, что ознакомился с Правилами сервиса</div>
            <GradientButton onClick={onSend}>Вывести ${q ? q.total.toFixed(0) : amount}</GradientButton>
        </div>
    )
}