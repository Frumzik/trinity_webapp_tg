...
export default function ScrollPanel({
    ...
    return (
        <div className={['sp gg', className].filter(Boolean).join(' ')} style={styleVars}>
            <div className="sp__box" style={{ maxHeight }} ref={boxRef} onScroll={recalc}>
                <div className="sp__content">{children}</div>
            </div>
            {showRail && <div className="sp__rail" aria-hidden />}
            <div className="sp__thumbY" style={{ height: `${thumb.h}px`, transform: `translateY(${thumb.y}px)` }} aria-hidden />
        </div>
    )
}
