...
export type UpdateProfileBody = Partial<Pick<User,'name'|'username'|'birthDate'|'height'|'weight'|'gender'|'avatarUrl'>>
...
export const userApi = createApi({
  ...
  endpoints: (b) => ({
    getUser: b.query<GetUserRes, { populate?: boolean } | void>({
      query: (p) => ({ url: '/user', params: { populate: p?.populate ?? true } }),
      providesTags: ['User'],
    }),
    ...
    updateFinPassword: b.mutation<{ success: true }, UpdateFinPasswordBody>({
      query: (body) => ({ url: '/user/update/fin-password', method: 'POST', body }),
    }),
  }),
})
