...
type RegisterTgReq = { type: 'TG'; tgId: number; pin: string; username?: string; name?: string }
type RegisterTgRes = { userId: number }
type LoginTgReq = { type: 'TG'; tgId: number; pin: string }
type LoginRes = { access_token: string }
type CheckTgRes = { exists: boolean } | boolean
...
export const authApi = createApi({
  ...
  endpoints: (b) => ({
    checkTg: b.query<CheckTgRes, number>({
      query: (tgId) => ({ url: `/auth/check-tg`, params: { tgId } }),
    }),
...
export const { useLazyCheckTgQuery, useRegisterTgMutation, useLoginTgMutation, useLazyCheckAuthQuery } = authApi