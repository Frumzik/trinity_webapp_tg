...
type Props = {
    ...
    onClose: () => void
    onPurchase: (payload: { levelIds: (string|number)[]; totalOM: number; discountedOM?: number }) => void
...
export default function LevelPurchaseModal({
    ...
    const selectable = useMemo(() => lockedLevels.filter(l => !l.purchased), [lockedLevels])

    const priceById = useMemo(
        () => new Map<string | number, number>(lockedLevels.map(l => [l.id, l.priceOM])),
        [lockedLevels]
    )
    ...
    const fullSum = useMemo(() => selectable.reduce((acc, l) => acc + l.priceOM, 0), [selectable])
        ...
        <div className={clsx('lp-modal', open && 'is-open')}>
            ...
            <div className="lp-sheet" role="dialog" aria-modal="true">
                <div className="lp-head">
                    ...
                    <button className="lp-close" onClick={onClose} aria-label="Закрыть">✕</button>
                </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="lp-cell-bottom">
                                    {!isBought && l.oldPriceOM && <span className="lp-old-mini">{l.oldPriceOM} OM</span>}
                                    {!isBought && <span className="lp-price">{l.priceOM} OM</span>}
                                </div>

                                {isBought && <span className="lp-badge"><span className="lp-badge__dot" /><svg xmlns="http://www.w3.org/2000/svg" width="12" height="10" viewBox="0 0 12 10" fill="none">
<path d="M2 4.5L5 7.5L10.5 2" stroke="white" stroke-width="3"/>
</svg></span>}
                            </button>
                        )
                    })}
                ...
                {selectable.length > 1 && (
                    <div className="lp-bulk">
                        ...
                        <span className="lp-note">при покупке <span style={{fontWeight: "500",color: "#474747",
                        }}>{selectable.length} ступеней сразу</span></span>
                    </div>
                )}
                        ...
                        <label className="lp-check">
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setCheckAll(val)
                                    setSelected(val ? selectable.map(l => l.id) : (defaultSelectedId ? [defaultSelectedId].filter(id => selectable.find(s => s.id === id)) : []))
                                }}
                            />
