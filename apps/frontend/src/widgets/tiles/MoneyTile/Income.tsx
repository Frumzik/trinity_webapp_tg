...
export default function IncomeCard({
    ...
    const cls = ['incomeCard', empty ? 'incomeCard--empty' : '', to || href || onClick ? 'is-clickable' : '', className].filter(Boolean).join(' ')
    const [m, k, u] = triads3(amountOM)

    return (
        <TileWrapper to={to} href={href} onClick={onClick} className={cls} ariaLabel={ariaLabel}>
            {imageUrl ? <div className="incomeCard__img" style={{ backgroundImage: `url(${imageUrl})` }} /> : null}
